    ...
    with engine.begin() as connection:
        ...
        if "bookings" in existing_tables:
            restored = connection.execute(
                text(
                    "UPDATE bookings "
                    "SET paid_amount = 10000, status = 'paid', payment_deadline = NULL "
                    "WHERE id = 16 AND COALESCE(paid_amount, 0) = 0 "
                    "AND status = 'awaiting_payment_confirmation'"
                )
            )
            if restored.rowcount:
                logging.info("Restored confirmed partial payment for booking #16")
